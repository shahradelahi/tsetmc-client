...
export default async function getWatchPrice(params?: GetWatchPriceParams, options: RequestOptions = {}): Promise<SafeReturn<WatchPrice[]>> {
  try {
    ...
    const {
      data: response,
      error
    } = await request('http://old.tsetmc.com/tsev2/data/MarketWatchPlus.aspx', deepmerge<RequestOptions>({
      params: {
        h: hEven,
        r: refId
      }
