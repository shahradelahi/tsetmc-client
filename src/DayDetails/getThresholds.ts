...
export default async function getThresholds(params: GetDayDetailsCommonParams, options: RequestOptions = {}): Promise<SafeReturn<ThresholdsData>> {
  try {
    ...
    const {
      data: response,
      error
    } = await request(`http://cdn.tsetmc.com/api/MarketData/GetStaticThreshold/${insId}/${dEven}`, options)
