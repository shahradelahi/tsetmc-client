...
export default async function getPriceOverviewData(params: GetDayDetailsCommonParams, options: RequestOptions = {}): Promise<SafeReturn<PriceOverviewData>> {
  try {
    ...
    const {
      data: response,
      error
    } = await request(`http://cdn.tsetmc.com/api/ClosingPrice/GetClosingPriceDaily/${insId}/${dEven}`, options)
