...
export default async function getOrderBook(params: GetDayDetailsCommonParams, options: RequestOptions = {}): Promise<SafeReturn<OrderBookDataRow[]>> {
  try {
    ...
    const {
      data: response,
      error
    } = await request(`http://cdn.tsetmc.com/api/BestLimits/${insId}/${dEven}`, options)
